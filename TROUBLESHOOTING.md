...
It is my assessment that an old version of a mod I am using (which shall remain unnamed) is causing the Papyrus scripting engine to back up and dump stacks. This problem is not common to everyone, but appears to only come into play when using the Craftloot and Resource Management features of the LOTD museum. The presence of Curator's Companion only makes this problem more evident, and that mod will likely be removed in the future. Until I am able to track down and test this issue more thoroughly, it is recommended to avoid the crafting systems in LOTD as well as the Universal Storage System until the modlist can be revamped. The issue appears to affect those with older CPUs or low framerates most frequently. This is an unrecoverable error, the only solution is to load the game before the sex scenes bugged out and try to be more careful with your save. 
